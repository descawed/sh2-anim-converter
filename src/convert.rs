...
impl AnimationConverter {
    ...
    pub fn convert(&self, output_skeleton: &Skeleton, output_path: &Path, use_model_transforms: bool) -> Result<()> {
        let mut file = File::create(output_path)?;
        self.input_animation.write_for_skeleton(output_skeleton, self.mapping.skeleton_mapping.as_slice(), use_model_transforms, &mut file)?;
